...
export default function App() {
  ...
  async function loadTracker() {
    ...
    try {
      const r = await window.storage.get("usa_event_tracker_v3", true);
      if (r?.value) setTracker(JSON.parse(r.value));
  ...
  async function persist(list) {
    setTracker(list);
    try { await window.storage.set("usa_event_tracker_v3", JSON.stringify(list), true); }
  ...
  const runSearch = useCallback(async () => {
    ...
    const sys = `You are an event discovery agent. Find real upcoming in-person tech events in the USA.
Search meetup.com, lu.ma, eventbrite, and conference sites for real events.
Return ONLY a valid JSON array. Each object must have:
name, date (use format "Month Day, Year" e.g. "March 25, 2026"), time, location, organizer,
source (Luma|Meetup|Conference|Eventbrite|Other),
type (Meetup|Conference|Hackathon|Networking|Workshop|Summit),
description (2-3 sentences), topics (comma-separated), url, tier (1=must-attend, 2=recommended, 3=optional),
relevance (one sentence on why this event matters).
No markdown fences. No explanation. Only the JSON array.`;

    const msg = `Find upcoming in-person tech events in ${city.trim()} related to: ${kw}.
Search: "${kw} events ${city.trim()} 2026", "lu.ma ${city.trim()} tech", "meetup.com technology ${city.trim()}", "eventbrite tech ${city.trim()}".
Return only the JSON array of real events you find.`;

    try {
      log("Querying live sources...");
      ...
      let ac = d1.content || [];
      for (const b of ac) if (b.type === "tool_use") log(`🔎 ${b.input?.query}`);

      let text = "";
      let messages = [{ role: "user", content: msg }, { role: "assistant", content: ac }];
      let currentData = d1;
      let rounds = 0;
      while (currentData.stop_reason === "tool_use" && rounds < 3) {
        rounds++;
        log(`Processing search results (pass ${rounds})...`);
        ...
        for (const b of dN.content || []) {
          if (b.type === "tool_use") log(`🔎 ${b.input?.query}`);
          if (b.type === "text") text += b.text;
        }
        messages.push({ role: "assistant", content: dN.content });
        currentData = dN;
      }
      if (!text) text = ac.filter(b => b.type === "text").map(b => b.text).join("");
          ...
          <>
            {loading && (
              <div style={{ textAlign: "center", padding: "48px 20px" }}>
                ...
                <div style={{ fontSize: 11, color: "#5c5c72", marginBottom: 16 }}>Meetup · Luma · Eventbrite · Conference sites</div>
                <div style={{
